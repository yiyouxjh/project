...
export class GameScene extends Phaser.Scene {
  ...
  _setupEvents() {
    ...
    this.events.on('enemyDeath', (x, y, score, type) => {
      ...
      // 道具掉落概率 = 敌方数量 / 30（原10减2/3）
      const dropChance = Math.min(this.enemies.length / 30, 0.27);
  ...
  _startWave(waveNumber) {
    ...
    // 每关开始时生成保底道具（每种1个）
    const types = ['weapon', 'shield', 'bomb', 'score', 'life'];
    let delay = 500;
    for (const type of types) {
      for (let i = 0; i < 1; i++) {
        const x = Phaser.Math.Between(40, GAME_CONFIG.width - 40);
        const pu = this.powerUpFactory.spawn(x, -30, type);
        pu.speed = 2 + Math.random() * 0.5;
  ...
  update() {
    ...
    for (let i = this.powerUps.length - 1; i >= 0; i--) {
      ...
      if (p.isDead || !p.active) continue;
      // 使用道具的实际位置
      const px = p.gfx ? p.gfx.x : p.x;
      const py = p.gfx ? p.gfx.y : p.y;
      const dx = px - this.player.x;
      const dy = py - this.player.y;
      const dist = Math.sqrt(dx*dx + dy*dy);
      if (dist < 80) {
        console.log('Collected:', p.type, 'dist:', dist.toFixed(1), 'gfx exists:', p.gfx !== null);
        p.applyToPlayer(this.player);
        console.log('After destroy, gfx null:', p.gfx === null);
